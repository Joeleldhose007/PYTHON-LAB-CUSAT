...
listOfVehicles = list()
vehicle_attributes = 
["ownerName","vendor","model","type","registrationNumber","engineNumber","mileage"]
vehicleDetails = dict.fromkeys(vehicle_attributes, None)

def addList():
    global listOfVehicles
    treeList.insert(parent='', index='end', text="", values=(owner.get(), 
    vendor.get(),model.get(),typeClass.get(),regNumber.get(),engNumber.
    get(),mileage.get()))
...
def loadFile():
    ...
    global listOfVehicles
    filename = askopenfilename(title="Open Pickle",initialdir='/',
    filetypes=filetypes)
...
def createPickle():
    ...
    file = asksaveasfile(filetypes = fileextensions, defaultextension = 
    fileextensions)
...
treeList = Treeview(columns=("Owner","Vendor","Model","Type","Reg Number","Eng 
Number","Mileage"),show='headings')
